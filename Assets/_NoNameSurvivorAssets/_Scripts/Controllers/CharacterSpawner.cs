            ...
            if(equipNewGuns)
            {
                if(ItemSelection.Instance.guns.Count > 0)
                {
                    for (int i = 0; i < ItemSelection.Instance.guns.Count; i++)
                    {
                        ...
                        _charactersOnTheScene[i].transform.GetChild(2).GetComponent<GunShooter>().SetGunParameters(gun.cooldown, gun.damageAmount, gun.gunMesh, gun.gunColor, gun.gunLength);
                    }
